...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(imgFrameRef.current, { clipPath: 'inset(100% 0% 0% 0%)' })
      gsap.set(imgRef.current, { scale: 1.25, yPercent: -4 })
      ...
      .fromTo(roleRef.current,
        { opacity: 0, y: 16 },
        { opacity: 1, y: 0, duration: 0.9 }, 1.1)
      .fromTo(subtitleRef.current,
        { opacity: 0, y: 16 },
        { opacity: 1, y: 0, duration: 0.9 }, 1.3)
      .fromTo(tagRowRef.current?.children ?? [],
        { opacity: 0, y: 12 },
        { opacity: 1, y: 0, duration: 0.6, stagger: 0.08 }, 1.5)
      .fromTo(scrollLabelRef.current,
        { opacity: 0 },
        { opacity: 1, duration: 1 }, 1.9)

      // ===== SCROLL PARALLAX (no clip-path involved → no flicker) =====
      ...
      // fade/move the whole wrapper instead of the clipped element itself
  ...
  return (
    <section ref={sectionRef} className="relative h-[180vh]" id="hero">
      <div className="sticky top-0 h-screen overflow-hidden bg-background">
        ...
        <div
          ref={lineTopRef}
          className="absolute top-0 left-0 right-0 h-px bg-white/10 origin-center z-[3]"
        />

        {/* corner labels */}
        <div className="absolute top-8 left-8 z-[5] hidden md:block" style={{ opacity: 0.35 }}>
          <span className="text-white text-[10px] tracking-[0.3em] uppercase" style={{ fontFamily: 'Inter, sans-serif' }}>
            №01 — Portfolio
          </span>
        </div>
        ...
        {/* layout: text one side, portrait other side */}
        <div className="relative z-[4] h-full max-w-7xl mx-auto px-6 md:px-12 grid md:grid-cols-2 items-center gap-8">

          {/* TEXT COLUMN */}
          <div
            className={isRTL ? 'order-1 md:order-2 text-center md:text-right' : 'order-1 text-center md:text-left'}
          >
            <div ref={roleRef} className="flex items-center justify-center md:justify-start gap-2 mb-6" style={{ opacity: 0 }}>
              ...
              >
                {t('در دسترس برای همکاری', 'Available for work')}
              </p>
            </div>
            ...
            <div ref={subtitleRef} style={{ opacity: 0 }}>
              ...
              >
                {t('ساختن تجربه‌های وب که حس خوبی دارن', 'Crafting web experiences that feel right')}
              </p>
            </div>
            ...
            <div ref={tagRowRef} className="flex flex-wrap items-center justify-center md:justify-start gap-2">
              {['React', 'TypeScript', 'GSAP'].map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1.5 rounded-full border border-white/10 text-white/45 text-[11px] tracking-wide"
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
                  {tag}
                </span>
          ...
          <div className={isRTL ? 'order-2 md:order-1' : 'order-2'}>
            <div className="relative mx-auto" style={{ width: 'min(82vw, 360px)' }}>
              ...
              >
                <img
                  ...
                  style={{
                    ...
                    filter: 'grayscale(20%) contrast(1.08) brightness(0.98)',
                  }}
                />
                ...
                {/* small caption chip on the image */}
                <div
        ...
        <div
          ref={scrollLabelRef}
          className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-[5]"
          style={{ opacity: 0 }}
        >
